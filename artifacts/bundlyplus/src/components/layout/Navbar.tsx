...
export function Navbar() {
  ...
  return (
    <nav style={{ top: 'max(env(safe-area-inset-top, 0px), 12px)' }} className={`fixed sm:!top-6 left-1/2 -translate-x-1/2 w-[94%] max-w-6xl z-50 rounded-full px-3 sm:px-6 py-2.5 sm:py-3 flex justify-between items-center gap-2 transition-all duration-300 ${
      scrolled
        ? 'bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-white/80 dark:border-white/10 shadow-xl shadow-purple-900/10 dark:shadow-black/30'
        : 'glass-panel'
    }`}>
